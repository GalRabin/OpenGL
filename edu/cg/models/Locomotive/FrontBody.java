...
public class FrontBody implements IRenderable {
    ...
    public void render() {
        glPushMatrix();
        // TODO-DONE(6): Render each part along with affine transformations in order to bring every component to the
        ...
        // Render right front wheel
        glPushMatrix();
        glTranslated(-Specification.BACK_BODY_WIDTH/2,-Specification.FRONT_BODY_HEIGHT/2,Specification.WHEEL_RADIUS/2);
        wheel.render();
        glPopMatrix();

        // Render left front wheel
        glPushMatrix();
        glTranslated(Specification.BACK_BODY_WIDTH/2,-Specification.FRONT_BODY_HEIGHT/2,Specification.WHEEL_RADIUS/2);
        wheel.render();
        glPopMatrix();

        // Render right front light
        glPushMatrix();
        glTranslated(-Specification.BACK_BODY_WIDTH/4,0,Specification.FRONT_BODY_DEPTH/2);
        carLight.render();
        glPopMatrix();

        // Render left front light
        glPushMatrix();
        glTranslated(Specification.BACK_BODY_WIDTH/4,0,Specification.FRONT_BODY_DEPTH/2);
        carLight.render();
        glPopMatrix();
